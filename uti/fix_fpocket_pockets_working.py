# 22/02/2021
# Author:  David McDonald & Sadettin Y. Ugurlu
...
def neigh_residue(pocket_name,protein_input,input_path_pocket,input_path_protein,output_path):
    ...
    print(pocket_name)
    print(protein_input)
    print(input_path_pocket)
    print(input_path_protein)
    print(output_path)
    ...
    for pocket_residue in pocket_structure.get_residues():
            ...
            # The pocket size can be adjustable to have more informative sequence-based features
            # Residue IDs +/- 1, +/- 2, +/- 3 genişletme
            for shift in [-5, -4, -3, -2, -1, 1, 2, 3, +4, +5]:
                if (int(residue_id) + shift) in all_residue_list:
                    residue_ids.append((chain_id, int(residue_id) + shift))
    os.chdir(input_path_protein)
    # set target structure
    parser = PDBParser()
